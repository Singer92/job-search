...
def fetch_rss_feeds(cfg, session):
    ...
    for feed in rss_cfg.get("feeds", []):
        try:
            ...
            for entry in parsed.entries[:10]: # Check latest 10 items per feed
                title = entry.get("title", "")
                link = entry.get("link", "")
                summary = strip_html(entry.get("summary", ""))
                # Pre-filter to save AI quota: only keep if it has music/arts keywords
...
def fetch_tn_gov_jobs(session):
    ...
    for target in targets:
        try:
            ...
            if r.status_code == 200:
                # Lightweight regex scraper for links containing music keywords
    ...
    # Deduplicate links found in the same run
